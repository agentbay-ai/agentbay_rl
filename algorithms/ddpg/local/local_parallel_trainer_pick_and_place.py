...
# ─────────────────────────────────────────────
# 环境配置常量（FetchPickAndPlaceDense-v4）
# ─────────────────────────────────────────────
ENV_NAME          = "FetchPickAndPlaceDense-v4"

OBS_DIM           = 25   # observation 维度（Pick&Place 包含物体位置、速度、旋转等信息）
GOAL_DIM          = 3    # achieved_goal / desired_goal 维度（目标物体位置 xyz）
STATE_DIM         = OBS_DIM + GOAL_DIM   # 输入网络的状态维度 = 28
ACTION_DIM        = 4    # 动作维度（末端执行器 dx, dy, dz + 夹爪开合）
MAX_ACTION        = 1.0
MAX_STEPS         = 50   # FetchPickAndPlace 每个 episode 最多 50 步

# FetchPickAndPlace 环境的超参数（参考 Notebook 配置）
DEFAULT_GAMMA     = 0.95   # 折扣因子（Notebook 推荐值，比 FetchReach 的 0.99 更低）
DEFAULT_TAU       = 0.05   # 目标网络软更新率（Notebook 推荐值，比 FetchReach 的 0.005 更高）
...
def make_env(env_name: str, render_mode: str = None):
    ...
    Args:
        env_name: 环境名称
        render_mode: 渲染模式，可选 "human"（窗口显示）、"rgb_array"、None（不渲染）
    """
    import gymnasium
...
class LocalWorker:
    ...
    def __init__(self, worker_id: int, env_name: str, agent: DDPGAgent,
        ...
        self.render_mode = render_mode  # 渲染模式：None / "human" / "rgb_array"
        self.env         = None

    def init_env(self):
        """初始化环境（线程内调用）"""
        self.env = make_env(self.env_name, render_mode=self.render_mode)
...
class LocalParallelTrainer:
    ...
    推荐配置（FetchPickAndPlaceDense-v4 能看到明显收敛）：
    ...
    def __init__(
        ...
        log_interval:         int   = 100,     # 日志输出间隔（轮数）
        gamma:                float = DEFAULT_GAMMA,   # 折扣因子（Pick&Place 推荐 0.95）
        tau:                  float = DEFAULT_TAU,     # 目标网络软更新率（Pick&Place 推荐 0.05）
        render:               bool  = False,   # 是否开启渲染（第一个 worker 显示画面）
    ):
        """
        参数说明:
            ...
            log_interval:         日志输出间隔（轮数，默认 100）
            gamma:                折扣因子（默认 0.95，FetchPickAndPlace 推荐值）
            tau:                  目标网络软更新率（默认 0.05，FetchPickAndPlace 推荐值）
            render:               是否开启渲染（默认 False，开启后第一个 worker 显示画面）
        """
    ...
    def _init_agent(self):
        ...
        print(f"   Batch Size: {self.agent.batch_size}, Gamma: {self.gamma}, Tau: {self.tau}")

    def _create_workers(self):
        # 只有第一个 worker 渲染（如果开启），其他 worker 不渲染
        self.workers = []
        for i in range(self.num_workers):
            # 第一个 worker 使用渲染模式（如果启用）
            render_mode = "human" if (self.render and i == 0) else None
            worker = LocalWorker(
                i, self.env_name, self.agent, self._read_lock,
                render_mode=render_mode
            )
            self.workers.append(worker)
        
        # 提前初始化所有环境（顺序，避免 mujoco 的并发初始化问题）
        render_info = "（第一个环境开启渲染）" if self.render else ""
        print(f"⚙️  初始化 {self.num_workers} 个本地环境...{render_info}")
    ...
    def _run_parallel_round(self, noise_scale: float) -> List[Dict]:
        ...
        注意：当启用渲染时，使用顺序执行而非多线程并行，
        因为 MuJoCo 渲染在多线程环境中可能会出现问题。
        """
        results = []
        
        if self.render:
            # 渲染模式：顺序执行（避免 MuJoCo 渲染的多线程问题）
            for w in self.workers:
                try:
                    results.append(w.run_episode(noise_scale))
                except Exception as e:
                    print(f"   ⚠️ Worker {w.worker_id} episode 执行失败: {e}")
        else:
            # 非渲染模式：多线程并行执行
    ...
    def train(self) -> Dict[str, Any]:
        ...
        print(f"   Gamma (折扣因子): {self.gamma}")
        print(f"   Tau (软更新率):   {self.tau}")
        print(f"   渲染:             {'开启' if self.render else '关闭'}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="本地并行 DDPG 训练器 (FetchPickAndPlaceDense-v4 环境，对齐 SB3 参数)"
    )
    ...
    parser.add_argument("--log-interval", type=int, default=100,
                        help="日志输出间隔（轮数）(默认: 100)")
    parser.add_argument("--gamma",     type=float, default=DEFAULT_GAMMA,
                        help=f"折扣因子 (默认: {DEFAULT_GAMMA}，FetchPickAndPlace 推荐值)")
    parser.add_argument("--tau",       type=float, default=DEFAULT_TAU,
                        help=f"目标网络软更新率 (默认: {DEFAULT_TAU}，FetchPickAndPlace 推荐值)")
    parser.add_argument("--render",    action="store_true",
                        help="开启渲染（第一个环境显示训练画面）")
